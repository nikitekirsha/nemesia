...
A concrete component extends `Component(name)`. Fields can request refs and options immediately because Nemesia prepares those APIs before subclass field initializers run.
...
Pass `observe: true` to watch for components added later:
