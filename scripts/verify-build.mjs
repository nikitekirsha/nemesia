...
function verifyEsmApi(api) {
  ...
  check(
    typeof api.Nemesia.DistributedComponent === 'function',
    `${label} is missing Nemesia.DistributedComponent`,
  )
...
function verifyUmdApi(api) {
  ...
  check(
    typeof api.DistributedComponent === 'function',
    `${label} is missing DistributedComponent`,
  )
