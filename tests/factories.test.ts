...
type HiddenConcreteMetadata = import('../src/index.js').ConcreteMetadata
// @ts-expect-error Metadata is internal to component registration.
type HiddenDistributedMetadata = import('../src/index.js').DistributedMetadata
// @ts-expect-error Metadata is internal to component registration.
type HiddenComponentMetadata = import('../src/index.js').ComponentMetadata
// @ts-expect-error Root inference helpers are internal to the factory.
type HiddenRootFor = import('../src/index.js').RootFor<'form'>
// @ts-expect-error Factory return helpers are not package-root API.
type HiddenAbstractConcreteConstructor = import('../src/index.js').AbstractConcreteComponentConstructor
// @ts-expect-error Factory return helpers are not package-root API.
type HiddenAbstractDistributedConstructor = import('../src/index.js').AbstractDistributedComponentConstructor
// @ts-expect-error Precise concrete constructors are internal factory types.
type HiddenConcreteConstructor = import('../src/index.js').ConcreteComponentConstructor
// @ts-expect-error Distributed constructors are internal factory types.
type HiddenDistributedConstructor = import('../src/index.js').DistributedComponentConstructor
...
describe('concrete component factory', () => {
  ...
  it('allows asynchronous lifecycle hooks', () => {
    ...
    expectTypeOf<BaseComponent['onMount']>().toEqualTypeOf<
      (() => void | Promise<void>) | undefined
    >()
    expectTypeOf<BaseComponent['onDestroy']>().toEqualTypeOf<
      (() => void | Promise<void>) | undefined
    >()
  })
...
describe('distributed component factory', () => {
  ...
  it('allows asynchronous lifecycle hooks', () => {
    class AsyncDistributedComponent extends Nemesia.DistributedComponent(
      'async-distributed-component',
    ) {
      async onMount(): Promise<void> {}
      async onDestroy(): Promise<void> {}
    }
    ...
    expectTypeOf(instance.onMount).toEqualTypeOf<() => Promise<void>>()
    expectTypeOf(instance.onDestroy).toEqualTypeOf<() => Promise<void>>()
    expectTypeOf<BaseDistributedComponent['onMount']>().toEqualTypeOf<
      (() => void | Promise<void>) | undefined
    >()
    expectTypeOf<BaseDistributedComponent['onDestroy']>().toEqualTypeOf<
      (() => void | Promise<void>) | undefined
    >()
  })
})
...
describe('public component types', () => {
  ...
  it('stores components created from union root tags', () => {
    const tag: 'form' | 'button' = document.body.matches('form')
      ? 'form'
      : 'button'
  ...
  it('keeps factories on the Nemesia namespace only', () => {
    ...
    type HasNamedComponent = 'Component' extends keyof PackageValues
      ? true
      : false
    type HasNamedDistributedComponent =
      'DistributedComponent' extends keyof PackageValues ? true : false
