...
describe('exact warnings', () => {
  ...
  it('merges user warning payloads after concrete and distributed context without changing lifecycle state', () => {
    ...
    expect(warn).toHaveBeenNthCalledWith(
      1,
      '[Nemesia] Component "user-distributed": distributed warning',
      { component: 'user-distributed', scope, detail: 2 },
    )
    expect(warn).toHaveBeenNthCalledWith(
      2,
      '[Nemesia] Component "user-distributed": distributed merge',
      { component: 'payload-distributed', scope: replacementScope },
    )
    expect(warn).toHaveBeenNthCalledWith(
      3,
      '[Nemesia] Component "user-concrete": plain warning',
      { component: 'user-concrete', root: concreteRoot, detail: 1 },
    )
    expect(warn).toHaveBeenNthCalledWith(
      4,
      '[Nemesia] Component "user-concrete": merged warning',
      { component: 'payload-component', root: replacementRoot },
    )
...
describe('exact unexpected error diagnostics', () => {
  it('uses console.error for concrete construction, onMount, and onDestroy failures', () => {
    ...
    expect(error).toHaveBeenNthCalledWith(
      1,
      '[Nemesia] Component "construction-error" failed during construction.',
      { component: 'construction-error', root: constructionRoot, error: constructionError },
    )
    expect(error).toHaveBeenNthCalledWith(
      2,
      '[Nemesia] Component "mount-error" failed during onMount.',
      { component: 'mount-error', root: mountRoot, error: mountError },
    )
    expect(error).toHaveBeenNthCalledWith(
      3,
      '[Nemesia] Component "destroy-error" failed during onDestroy.',
      { component: 'destroy-error', root: destroyRoot, error: destroyError },
    )
  })
  ...
  it('includes the exact distributed scope and original errors', () => {
    ...
    expect(error).toHaveBeenNthCalledWith(
      ...
      { component: 'distributed-construction', scope, error: constructionError },
    )
    expect(error).toHaveBeenNthCalledWith(
      2,
      '[Nemesia] Component "distributed-mount" failed during onMount.',
      { component: 'distributed-mount', scope, error: mountError },
    )
    expect(error).toHaveBeenNthCalledWith(
      3,
      '[Nemesia] Component "distributed-destroy" failed during onDestroy.',
      { component: 'distributed-destroy', scope, error: destroyError },
    )
  })
  ...
  it('keeps warning and error diagnostics observational when console methods throw', () => {
    ...
    class Good extends Nemesia.Component('observational-good') {
      onMount(): void { mounted.push('good') }
    }
    ...
    expect(() => createApp()
      .register([Original, Latest, Broken, Good])
      .mount(document.body)).not.toThrow()
