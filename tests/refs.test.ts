...
type HiddenRefApi = Api.RefApi
// @ts-expect-error Controlled mount errors are internal.
type HiddenSkipComponentMountError = Api.SkipComponentMountError
...
function appendRef<TElement extends Element>(root: Element, element: TElement, name: string): TElement {
...
describe('ref façade', () => {
	...
	it('supports generic required and optional refs for HTML and SVG elements', () => {
		...
		const svg = appendRef(root, document.createElementNS('http://www.w3.org/2000/svg', 'svg'), 'svg')
		const ref = createRefApi(root, 'example')
...
describe('ref discovery and ownership', () => {
	it('resolves valid elements created by another Window realm', () => {
		...
		const container = appendRef(root, foreignDocument!.createElement('div'), 'container')
		const button = appendRef(root, foreignDocument!.createElement('button'), 'button')
		const svg = appendRef(root, foreignDocument!.createElementNS('http://www.w3.org/2000/svg', 'svg'), 'drawing')
...
describe('controlled ref mount failures', () => {
	it('reports an exact controlled error for a missing required single ref', () => {
		...
		expectControlledError(() => createRefApi(root, 'slider').button('swiper'), 'missing required ref "swiper"', {
			component: 'slider',
			root,
			ref: 'swiper',
	...
	it('reports an exact controlled error for a missing required many ref', () => {
		...
		expectControlledError(() => createRefApi(root, 'list').many.element('item'), 'missing required ref "item"', {
			component: 'list',
			root,
			ref: 'item',
	...
	it('rejects SVG elements through the HTMLElement helper', () => {
		...
		appendRef(root, document.createElementNS('http://www.w3.org/2000/svg', 'svg'), 'drawing')
	...
	it('makes no console warning while resolving refs', () => {
		...
		expect(() => createRefApi(root, 'example').button('missing')).toThrow(SkipComponentMountError)
