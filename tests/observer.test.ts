...
describe('mutation root normalization', () => {
	it('deduplicates roots, drops descendants, expands fragments, and preserves retained order', () => {
		...
		expect(
			normalizeMutationRoots([child, sibling, parent, child, document.createTextNode('ignored'), fragment])
		).toEqual([sibling, parent, fragmentRoot])
	})

	it('normalizes a large sibling batch without pairwise containment scans', () => {
		const roots = Array.from({ length: 250 }, () => document.createElement('div'))
...
describe('automatic concrete destruction and batching', () => {
	...
	it('destroys historically nested roots extracted across documents before delivery', async () => {
		...
		const app = createApp({ observe: true }).register([HistoricalParent, HistoricalChild])
		app.mount(scope)
	...
	it('remounts a still-observed exact scope detached from its observed outer scope', async () => {
		...
		outer.append(inner)
		document.body.append(outer)
		const app = createApp({ observe: true }).register([DetachedConcrete, DetachedDistributed])
		app.mount(outer)
		app.mount(inner)
...
describe('observer lifecycle', () => {
	it('creates exactly one real observer per exact scope across repeated mounts', () => {
		...
		let constructed = 0
		vi.spyOn(window, 'MutationObserver').mockImplementation(function (callback: MutationCallback): MutationObserver {
			constructed += 1
			return new NativeMutationObserver(callback)
		} as unknown as (this: MutationObserver, callback: MutationCallback) => MutationObserver)
		const scope = document.createElement('main')
		document.body.append(scope)
	...
	it('disconnects one exact scope without destroying it and can observe it again after a later mount', async () => {
		...
		await flushMutations()
		expect(mounted).toEqual([existing, activeAddition, disconnectedAddition, reobservedAddition])
		expect(destroyed).toEqual([])
	})
