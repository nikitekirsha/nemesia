...
export class CodeMarkers extends Component('code-markers') {
	...
	private place() {
		...
		const size = this.markers[0]?.offsetWidth ?? 0
		const gutter = boxes.some(box => box !== undefined && box.right + GAP + size > origin.right)

		this.markers.forEach((marker, i) => {
			const box = boxes[i]
			if (box === undefined) return

			const left = gutter ? box.left - size / 2 - 2 : box.right + GAP + size / 2
