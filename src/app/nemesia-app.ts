...
import {
  beginComponentConstruction,
  endComponentConstruction,
} from '../internal/construction.js'
import {
  deepestFirst,
...
type DistributedConstructor = (new (
  scope: ParentNode,
...
export class NemesiaAppImplementation implements NemesiaApp {
  ...
  readonly #distributedInstances = new WeakMap<
    ParentNode,
    Map<string, BaseDistributedComponent>
  >()
  readonly #distributedConstructionReservations = new WeakMap<
    ParentNode,
    Set<string>
  >()
  readonly #observers = new WeakMap<ParentNode, MutationObserver>()
  readonly #observedScopes = new Set<ParentNode>()
  readonly #observedScopeAncestries = new WeakMap<
    ParentNode,
    readonly Node[]
  >()
  ...
  #destroyConcrete(scope: ParentNode): void {
    const roots = deepestFirst(
      [...this.#mountedRoots].filter(root => isWithinScope(root, scope)),
    )
  ...
  #destroyConcreteRoots(roots: Iterable<Element>): void {
    ...
    this.#observedScopeAncestries.set(
      scope,
      snapshotAncestry(scope as Node),
    )
  }
  ...
  #flushMutationBatch(): void {
    ...
    const removedRoots = normalizeRemovedMutationRoots(
      this.#pendingRemovedNodes.splice(0),
    )
    const addedRoots = normalizeMutationRoots(
      this.#pendingAddedNodes.splice(0),
    )

    const scopesToReconcile = this.#observedScopesHistoricallyWithin(
      removedRoots,
    )
  ...
  #destroyHistoricallyRemovedConcrete(removedRoots: Element[]): void {
    ...
    const roots = [...this.#mountedRoots].filter(root => (
      this.#mountedRootAncestries
        .get(root)
        ?.some(ancestor => removed.has(ancestor)) ?? false
    ))

    roots.sort((left, right) => (
      (this.#mountedRootAncestries.get(right)?.length ?? 0)
      - (this.#mountedRootAncestries.get(left)?.length ?? 0)
    ))
    this.#destroyConcreteRoots(roots)
  }

  #observedScopesHistoricallyWithin(
    removedRoots: Element[],
  ): ParentNode[] {
    const removed = new Set<Node>(removedRoots)

    return [...this.#observedScopes].filter(scope => (
      this.#observedScopeAncestries
        .get(scope)
        ?.some(ancestor => removed.has(ancestor)) ?? false
    ))
  }
  ...
  #isCurrentConcreteCandidate(
    root: Element,
    scope: ParentNode,
    observerDriven: boolean,
  ): boolean {
    return observerDriven
      ? this.#isWithinObservedScope(root)
      : isWithinScope(root, scope)
  }
  ...
  #mountConcrete(scope: ParentNode, observerDriven = false): void {
    for (const root of discoverConcreteRoots(scope)) {
      ...
      if (
        name === null
        || this.#state.isRootConstructing(root)
        || this.#instances.has(root)
      ) continue
      ...
      if (
        !component.nemesia.multiple
        && this.#state.hasActiveOrConstructing(name)
      ) {
        warnSkippedComponent(
          name,
          'only one instance may be mounted',
          { component: name, root },
        )
        continue
      }
      ...
      if (hookResult !== undefined) {
        void Promise.resolve(hookResult).catch(error => {
          this.#failMount(root, name, instance, error)
        }).catch(() => {
  ...
  #mountDistributed(scope: ParentNode): void {
    for (const registration of this.#registrations.values()) {
      ...
      if (
        this.#distributedInstances.get(scope)?.has(name)
        || this.#isDistributedConstructing(scope, name)
      ) continue
  ...
  #isDistributedConstructing(scope: ParentNode, component: string): boolean {
    return this.#distributedConstructionReservations
      .get(scope)
      ?.has(component) ?? false
  }

  #reserveDistributedConstruction(
    scope: ParentNode,
    component: string,
  ): void {
  ...
  #releaseDistributedConstruction(
    scope: ParentNode,
    component: string,
  ): void {
  ...
  #isValidRoot(
    ...
    if (metadata.root !== undefined && root.localName !== metadata.root) {
      warnSkippedComponent(
        component,
        `expected an HTML <${metadata.root}> root`,
        {
          component,
          root,
