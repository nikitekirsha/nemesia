...
const buttonType: ExpectedType<HTMLButtonElement> = {
  ...
  accepts: element =>
    element.namespaceURI === HTML_NAMESPACE && element.localName === 'button',
}
...
const textareaType: ExpectedType<HTMLTextAreaElement> = {
  ...
  accepts: element =>
    element.namespaceURI === HTML_NAMESPACE && element.localName === 'textarea',
}

const selectType: ExpectedType<HTMLSelectElement> = {
  name: 'HTMLSelectElement',
  accepts: element =>
    element.namespaceURI === HTML_NAMESPACE && element.localName === 'select',
}
...
export function createRefApi(root: HTMLElement, componentName: string): RefApi {
  ...
  const discover = (name: string): Element[] =>
    Array.from(root.querySelectorAll('[data-ref]')).filter(element =>
      element.getAttribute('data-ref') === name
      && element.closest('[data-nemesia]') === root,
    )
  ...
  const fail = (
    ...
    throw new SkipComponentMountError(
      reason,
      diagnosticPayload(name, details),
    )
  }
  ...
  function resolve<TElement extends Element>(
    ...
    if (!options.many && elements.length > 1) {
      return fail(
        `duplicate ref "${name}": expected one element, received ${elements.length}`,
        name,
        { expected: 'one element', received: elements.length },
      )
    }

    const expected = options.expected
    const invalid = expected
      ? elements.find(element => !expected.accepts(element))
      : undefined

    if (invalid && expected) {
      const received = receivedType(invalid)
      return fail(
        `invalid ref "${name}": expected ${expected.name}, received ${received}`,
        name,
        {
    ...
    return options.many
      ...
      : elements[0] as TElement
  }

  const requiredOne = <TElement extends Element>(
    name: string,
    expected?: ExpectedType<TElement>,
  ): TElement => resolve(name, { required: true, many: false, expected })

  const optionalOne = <TElement extends Element>(
    name: string,
    expected?: ExpectedType<TElement>,
  ): TElement | null => resolve(name, { required: false, many: false, expected })

  const requiredMany = <TElement extends Element>(
    name: string,
    expected?: ExpectedType<TElement>,
  ): TElement[] => resolve(name, { required: true, many: true, expected })

  const optionalMany = <TElement extends Element>(
    name: string,
    expected?: ExpectedType<TElement>,
  ): TElement[] => resolve(name, { required: false, many: true, expected })

  const optionalManyApi = {
    of: <TElement extends Element = HTMLElement>(name: string): TElement[] =>
      optionalMany<TElement>(name),
